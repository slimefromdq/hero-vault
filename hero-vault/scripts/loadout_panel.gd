...
func _ready() -> void:
	...
	for row in range(5):
		...
		for label in ["Top / solo", "Mid / river cut", "Bottom / cover", "Jungle / roam"]:
			order.add_item(label)
		order.get_popup().set_item_tooltip(0, "Isolated 1v1. Long walk for help. Narrow road, no cover.")
		order.get_popup().set_item_tooltip(1, "Short lane. Fast river cuts into top or bottom.")
		order.get_popup().set_item_tooltip(2, "Duo fights behind bushes. Cover blocks shots that try to pass through.")
		order.get_popup().set_item_tooltip(3, "Clear camps and rotate. Mid is the fastest path between sidelanes.")
	...
	make_button("Restore starter squad", Vector2(942, 656), Vector2(210, 42), func():
		...
		orders = Catalog.DEFAULT_LANES.duplicate()
		update_view())
	hide()
