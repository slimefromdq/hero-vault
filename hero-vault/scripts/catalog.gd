...
const DEFAULT_LANES := [0, 1, 2, 2, 3]
const ROAM_ORDER := 3
const LANE_LABELS := ["Top", "Mid", "Bot", "Roam"]
...
## Two-lane saves used 0=north, 1=south, 2=roam. Extra north slots become mid.
static func migrate_lanes(saved: Array) -> Array:
	var result: Array = DEFAULT_LANES.duplicate()
	if saved.size() != TEAM_SIZE:
		return result
	var highest := 0
	for i in range(TEAM_SIZE):
		highest = maxi(highest, int(saved[i]))
	if highest >= ROAM_ORDER:
		for i in range(TEAM_SIZE):
			result[i] = clampi(int(saved[i]), 0, ROAM_ORDER)
		return result
	var have_top := false
	for i in range(TEAM_SIZE):
		var value := clampi(int(saved[i]), 0, 2)
		if value == 2:
			result[i] = ROAM_ORDER
		elif value == 1:
			result[i] = 2
