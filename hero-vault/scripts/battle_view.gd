...
func draw_terrain() -> void:
	...
	var paving := [Color("2f3a3c"), Color("3a4640"), Color("343e41")]
	for lane in range(MapLayout.LANE_COUNT):
		var road := PackedVector2Array()
		for point in MapLayout.path(lane):
			road.append(arena_point(point))
		var width: float = MapLayout.half_width(lane)*1.64
		draw_polyline(road, Color("879085"), width+4, true)
		draw_polyline(road, paving[lane], width, true)
		for p in road:
			draw_circle(p, width*0.5, paving[lane])
		for progress in range(80, int(MapLayout.length(lane))-60, 70):
			var point := MapLayout.point_at(lane, progress)
			var tangent := MapLayout.forward(point, lane, 0)
			var center := arena_point(point)
			draw_line(center-tangent.orthogonal()*width*0.42, center+tangent.orthogonal()*width*0.42, Color("475050"), 1)
	for bush in MapLayout.COVER:
		var rect := Rect2(arena_point(bush.position), bush.size*0.82)
		draw_style_box(panel_style(Color("1b3328"), 8), rect)
		draw_arc(rect.get_center(), minf(rect.size.x, rect.size.y)*0.38, 0, TAU, 16, Color("3d6a4a"), 2, true)
	label_at("COVER", arena_point(Vector2(430, 455)), 10, Color("8fbf9a"))
	var jungle := panel_style(Color("153e2c"), 15)
	jungle.border_color = Color("729b72")
	jungle.set_border_width_all(3)
	for patch in MapLayout.JUNGLE_PATCHES:
		draw_style_box(jungle, Rect2(arena_point(patch.position), patch.size*0.82))
	for i in range(MapLayout.LANE_COUNT):
		draw_dashed_line(arena_point(MapLayout.ENTRANCES[i]), arena_point(MapLayout.JUNGLE_WAYPOINTS[0 if i != MapLayout.BOT else 1]), Color("456c43"), 9, 8)
	for i in range(23):
		var patch: Rect2 = MapLayout.JUNGLE_PATCHES[i%2]
		var point := patch.position+Vector2(12+(i*47)%maxi(20, int(patch.size.x-24)), 12+(i*31)%maxi(20, int(patch.size.y-24)))
		var center := arena_point(point)
	...
	label_at("JUNGLE", arena_point(Vector2(318, 168)), 12, Color("cef0af"))
	label_at("TOP / SOLO", arena_point(Vector2(335, 35)), 10, MUTED)
	label_at("MID / CUT", arena_point(Vector2(470, 210)), 10, MUTED)
	label_at("BOT / COVER", arena_point(Vector2(500, 474)), 10, MUTED)
