...
var lane_orders: Array = Catalog.DEFAULT_LANES.duplicate()
...
func draw_home() -> void:
	...
	for i in range(5):
		...
		label_at(Catalog.LANE_LABELS[lane_orders[i]],Vector2(x,419),11,MUTED)
...
func draw_game() -> void:
	...
	for i in range(5):
		...
		var place: String = "KO"
		if u.hp > 0:
			place = Catalog.LANE_LABELS[u.lane] if u.lane < Catalog.LANE_LABELS.size() else "Mid"
		label_at("L%d / %s" % [u.level, place],Vector2(98,y+34),11,MUTED)
...
func load_profile() -> void:
	...
	if value is Dictionary:
		...
		var saved_orders = value.get("lane_orders", Catalog.DEFAULT_LANES)
		if saved_orders is Array and saved_orders.size() == 5:
			lane_orders = Catalog.migrate_lanes(saved_orders)
