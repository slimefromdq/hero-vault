...
Three lanes, plus jungle roam. Default assignment: top, mid, bottom duo, roam (`Catalog.DEFAULT_LANES`).

| Lane | Shape | Role |
|---|---|---|
| **Top** | Long L, narrow (half-width 32) | Solo duels. Rotations from bottom take the long jungle walk. |
| **Mid** | Short diagonal | Quick duels. River cuts into either sidelane; roamers start here. Mid laners with roaming 3+ look in on a sidelane fight, then return. |
| **Bottom** | Long L, wide (half-width 54) | Duo fights. Four bushes: shots cannot pass through a bush to hit someone on the far side. Heroes in cover move slower. Waves are tankier (90 HP creeps). |

Old saves (`0` north, `1` south, `2` roam) migrate through `Catalog.migrate_lanes`.
...
`stitch`, `stitch_mirror`, `full_send_launch`, `full_send_land` (`detail` = hit/miss, value = heroes hit), `crash_program_shockwave`, `knockback`, `wall_slam`, `flare_burst` (`detail` = hit/miss), `cloak_reveal`, `cloak_gank_success`, `item_evolved`. Cover-blocked shots use `projectile_miss` with `detail` = `cover`. Answer "Where is Crash Test now?" with `full_send_*` positions, and "Do Sunday's slow attacks land?" with `projectile_hit`/`projectile_miss` plus `flare_burst`. Existing IDs, including `crown_*` for the Double Damage Idol, are unchanged.
